...
const UpdateBugComponent = ({ data, onUpdated }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    const { title, severity, progress, description, estimatedFixTimeHours } =
      formData;

    if (!title || !severity || !progress || !estimatedFixTimeHours) {
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div>
          ...
          <input
            ...
            className="w-full opacity-50 bg-gray-300 rounded-md border border-gray-300 px-3 py-2 text-sm focus:ring-1 focus:ring-blue-500 focus:outline-none"
          />
        </div>
